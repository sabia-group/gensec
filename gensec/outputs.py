...
class Directories:
    ...
    def save_to_directory(self, ensemble, parameters):
        dir = os.path.join(os.getcwd(), format(self.dir_num, "010d"))
        write(
            os.path.join(dir, "{:010d}.in".format(self.dir_num)),
            ensemble,
            format="aims",
        )
...
class Output:
    ...
    def write_parameters(self, parameters, structure, known, dirs):
        ...
        report.write(
            "If the unknown structure will not be found {} times in row the criteria for similarity between structures will be decreased.\n".format(
                parameters["trials"]
            )
        )
        report.write(
            "After {} structures will be relaxed the algorithm will stop.\n".format(
                parameters["success"]
            )
        )
        report.write(
            "{} replicas will be produced.\n".format(parameters["number_of_replicas"])
        )
        report.write(
            "Reading template geometry from {}.\n".format(parameters["geometry"][0])
        )
        report.write(
            "Number of atoms in template molecule is {}.\n".format(
                len(structure.molecules[0])
            )
        )
        if parameters["fixed_frame"]["activate"]:
            report.write(
                "Reading fixed frame from {}.\n".format(
        ...
        if parameters["mic"]["activate"]:
            ...
            report.write(
                "Sampling is performed without periodic boundary conditions.\n"
            )
        if parameters["configuration"]["torsions"]["list_of_tosrions"] == "auto":
            report.write(
                "Identified rotatable bonds are: {}.\n".format(
                    structure.list_of_torsions
                )
            )
        if parameters["configuration"]["torsions"]["activate"]:
            report.write(
                "{} values of torsions will be sampled.\n".format(
                    parameters["configuration"]["torsions"]["values"]
                )
            )
        if parameters["configuration"]["orientations"]["activate"]:
            report.write(
                "{} values of orientations will be sampled.\n".format(
                    parameters["configuration"]["orientations"]["values"]
                )
            )
        if parameters["configuration"]["coms"]["activate"]:
            report.write(
                "{} values of centres of masses will be sampled.\n".format(
                    parameters["configuration"]["coms"]["values"]
                )
            )
        report.write(
            "Folder with suporting files is in {}.\n".format(
                parameters["calculator"]["supporting_files_folder"]
            )
        )
        report.write(
            "ASE calculator is in {}.\n".format(
                parameters["calculator"]["ase_parameters_file"]
            )
        )
        if parameters["calculator"]["optimize"] == "generate":
            report.write(
                'GenSec wil generate structures in "generate" folder without relaxation\n'
            )
        elif "search" in parameters["calculator"]["optimize"]:
            report.write(
                'GenSec will generate structures in "generate" folder and relax them in {} folder\n'.format(
                    parameters["calculator"]["optimize"]
                )
            )
            report.write(
                "Relaxation will be perfomed until {} remaining forces are reached\n".format(
                    parameters["calculator"]["fmax"]
                )
            )
            report.write(
                "On molecular part {} preconditioner of the Hessian matrix will be applied\n".format(
                    parameters["calculator"]["preconditioner"]["mol"]
                )
            )
            report.write(
                "On fixed frame part {} preconditioner of the Hessian matrix will be applied\n".format(
                    parameters["calculator"]["preconditioner"]["mol"]
                )
            )
            report.write(
                "Between molecular parts {} preconditioner of the Hessian matrix will be applied\n".format(
                    parameters["calculator"]["preconditioner"]["mol"]
                )
            )
            report.write(
                "Between molecular part and fixed frame part {} preconditioner of the Hessian matrix will be applied\n".format(
                    parameters["calculator"]["preconditioner"]["mol"]
                )
            )
            report.write(
                "After difference in RMSD during geometry optimization reaches {} - updating of the Hessian will be performed\n".format(
                    parameters["calculator"]["preconditioner"]["rmsd_update"]
                )
            )
        ...
        for struc in range(1, dirs.dir_num + 1):
            if parameters["configuration"]["torsions"]["activate"]:
                report.write(
                    "Structure {} has torsional angles\n{}\n".format(
                        struc, known.torsions[struc]
                    )
                )
    ...
    def write_successfull_generate(self, parameters, configuration, dirs):
        ...
        dir = os.path.join(
            os.getcwd(),
            parameters["calculator"]["optimize"],
            format(dirs.dir_num, "010d"),
        )
        report.write(
            "Structure {} succsessfully generated and saved in \n{}\n".format(
                dirs.dir_num, dir
            )
        )
        report.write(
            "Structure {} has torsional angles configuration \n{}\n".format(
                dirs.dir_num, configuration
            )
        )
        report.close()
    ...
    def write_successfull_relax(self, parameters, structure, known, dirs):
        ...
        report.write(
            "Structure {} succsessfully relaxed and saved in \n{}\n".format(
                dirs.dir_num, dir
            )
        )
        tors = measure_torsion_of_last(
            Trajectory(os.path.join(dir, known.find_traj(dir)))[-1],
            structure.list_of_torsions,
        )
        report.write(
            "Local minima of structure {} has torsional angles configuration \n{}\n".format(
                dirs.dir_num, tors
            )
        )
        report.close()
