...
if "search" in parameters["calculator"]["optimize"]:
    ...
    output = Output(
        os.path.join(
            os.getcwd(), "report_{}.out".format(parameters["calculator"]["optimize"])
        )
    )
    ...
    calculator.finish_relaxation(
        structure, fixed_frame, parameters, dirs.current_dir(parameters)
    )
    workflow.success = dirs.dir_num
    generated_dirs = [
        z
        for z in os.listdir(dirs.generate_folder)
        if os.path.isdir(os.path.join(dirs.generate_folder, z))
    ]
        ...
        while workflow.success < parameters["success"]:
            ...
            if len(generated_dirs) > 0:
                ...
                try:
                    ...
                    gen = read(
                        os.path.join(d, sorted(generated_dirs)[0] + ".in"),
                        format="aims",
                    )
                    configuration = structure.read_configuration(
                        structure, fixed_frame, gen
                    )
                    shutil.rmtree(d)
                except:
                ...
                if not found:
                    ...
                    dirs.save_to_directory(
                        merge_together(structure, fixed_frame), parameters
                    )
                    calculator.relax(
                        structure,
                        fixed_frame,
                    ...
                    t = known.find_traj(os.path.join(dirs.current_dir(parameters)))
                    conf = measure_torsion_of_last(
                        Trajectory(os.path.join(dirs.current_dir(parameters), t))[-1],
                    ...
                    if not found:
                        ...
                        workflow.success += 1
                        workflow.trials = 0
                        output.write_successfull_relax(
                            parameters, structure, known, dirs
                        )
                        output.write_to_report(
                            "\nGenerated structure in folder {} is deleted\n".format(d)
                        )
            ...
            if all_right(structure, fixed_frame):
                ...
                if not found:
                    ...
                    structure.apply_configuration(
                        configuration
                    )  # For some reason need to apply again here
                ...
                write(
                    "bad_luck.xyz", merge_together(structure, fixed_frame), format="xyz"
                )
                pass
        else:
            output.write_to_report(
                "{} number of structures was successfully relaxed!".format(
                    parameters["success"]
                )
            )
        ...
        while workflow.success < parameters["success"]:
            ...
            if all_right(structure, fixed_frame):
                ...
                if not found:
                    ...
                    t, o, c = known.get_internal_vector(
                        current_coords, structure, fixed_frame, parameters
                    )
                    known.add_to_known(t, o, c)
                    workflow.success += 1
                    workflow.trials = 0
                    output.write_successfull_generate(
                        parameters, structure.torsions_from_conf(configuration), dirs
                    )
            ...
            output.write_to_report(
                "{} number of structures was successfully generated!\n".format(
                    parameters["success"]
                )
            )
            output.write_to_report("Terminating algorithm\n")
...
if "single" in parameters["calculator"]["optimize"]:
    ...
    if len(os.listdir(parameters["calculator"]["generate_folder"])) > 0:
        ...
        for i in sorted(os.listdir(gendir)):
            ...
            parameters["calculator"]["ase_parameters_file"] = os.path.join(
                basedir, "supporting", "ase_command.py"
            )
            ...
            structure.mu = np.abs(
                calculator.estimate_mu(structure, fixed_frame, parameters)
            )
            dirs.create_directory(parameters)
            dirs.save_to_directory(merge_together(structure, fixed_frame), parameters)
            calculator.relax(
                structure, fixed_frame, parameters, dirs.current_dir(parameters), known
            )
            dirs.finished(parameters)
